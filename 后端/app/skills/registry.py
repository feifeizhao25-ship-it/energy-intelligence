...
class SkillRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._aliases: Dict[str, str] = {}            # alias (如 F-001) → 主 skill_id
    ...
    def _register_skill(
        ...
        # 编号别名（运营规格 F 编号 → 技术 skill_id）；先注册者保留，冲突不覆盖
        for alias in getattr(instance, "aliases", None) or []:
            alias = str(alias)
            if alias in self._skills or alias in self._aliases:
                logger.debug("Alias %s for %s conflicts, skipping", alias, skill_id)
                continue
    ...
    def get(self, skill_id: str) -> Optional[SkillWrapper]:
        """获取已注册的 Skill（支持主 ID 与 aliases 中的编号别名）"""
    ...
    def get_meta(self, skill_id: str) -> Optional[SkillMeta]:
        """获取 Skill 元信息（支持主 ID 与 aliases 中的编号别名）"""
        ...
        canonical = self._aliases.get(skill_id)
        if canonical is not None:
            return self._meta.get(canonical)
