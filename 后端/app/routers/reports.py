...
DISCLAIMER_CN = "本内容仅供研究参考，不构成投资建议。投资有风险，决策需谨慎。"
DISCLAIMER_EN = "For research reference only. Not investment advice."
WATERMARK_TEXT_CN = "EnergyIQ 研究参考"          # 显式水印 (页脚浅色斜体/节级水印)
AI_REVIEW_LABEL_CN = "AI 辅助生成 + 人工审核"     # 封面标注
FORECAST_NOTE_CN = "（这是预测，非确定）"
FORECAST_NOTE_EN = "(This is a forecast, not a certainty.)"
GENERATOR_ID = "EnergyIQ-AIGC"                    # 隐式水印: 写入文档元数据

# 数据溯源表追踪的数据键 (键路径, 中文标签)
_PROVENANCE_KEYS = [
    ("name", "项目名称"),
    ("project_type", "项目类型"),
    ("capacity_mw", "装机容量"),
    ("province_or_region", "项目地点"),
    ("currency", "货币单位"),
    ("financial.initial_investment", "初始投资"),
    ("financial.annual_generation_mwh", "年发电量"),
    ("financial.electricity_price", "上网电价"),
    ("financial.discount_rate", "折现率"),
    ("financial.opex_annual", "年运维成本"),
]


def _data_provenance_rows(data: Dict[str, Any], lang: str = "cn") -> List[List[str]]:
    """数据溯源表行: [数据项, 来源标注].

    规则 (运营资料硬要求): demo 数据标「演示数据」, 缺失数据标「资料未及」,
    数据库项目数据标「项目数据库」; 英文分支用等价英文标注。
    """
    is_demo = data.get("data_source") == "demo"
    rows: List[List[str]] = []
    for key, label_cn in _PROVENANCE_KEYS:
        if key.startswith("financial."):
            value = data.get("financial", {}).get(key.split(".", 1)[1])
        else:
            value = data.get(key)
        if value in (None, "", 0, 0.0):
            source = "资料未及" if lang == "cn" else "not available"
        elif is_demo:
            source = "演示数据" if lang == "cn" else "demo data"
        else:
            source = "项目数据库" if lang == "cn" else "project database"
        rows.append([label_cn if lang == "cn" else key, source])
    return rows
...
def _build_project_dict(project: Any, cf: Any = None) -> Dict[str, Any]:
    ...
    data: Dict[str, Any] = {
        ...
        "data_source": "project_db",  # 溯源标注: 项目数据库
    }
...
def _global_section_body(section: str, data: Dict[str, Any], metrics: Dict[str, Any]) -> str:
    ...
    bodies = {
        ...
        "Market Analysis": (
            ...
            f"Forward-looking growth figures are forecasts. {FORECAST_NOTE_EN}"
        ),
        "Financial Model": (
...
def _generate_docx_global(
    ...
    # 隐式水印: 文档元数据写入 generator 标识与生成时间戳
    core = doc.core_properties
    core.author = GENERATOR_ID
    core.comments = (
    ...
    doc.add_heading("Data provenance", level=1)
    for item, source in _data_provenance_rows(data, lang="en"):
        doc.add_paragraph(f"{item}: {source}")
    ...
    # 固定免责声明 (底栏, 运营资料硬要求)
    doc.add_paragraph(DISCLAIMER_EN)
...
def _generate_pdf_global(
    ...
    doc = SimpleDocTemplate(
        ...
        title=title,
        # 隐式水印: PDF metadata 写入 generator 标识与生成时间戳
    ...
    story.append(Spacer(1, 3 * mm))
    story.append(Paragraph("Data provenance", styles["Heading1"]))
    for item, source in _data_provenance_rows(data, lang="en"):
        story.append(Paragraph(f"{item}: {source}", styles["Normal"]))
    story.append(Spacer(1, 3 * mm))
    story.append(Paragraph(
        "Disclaimer: forecasts depend on assumptions; actual results may differ. "
        "The authors accept no liability for decisions made on this report.",
    ...
    # 固定免责声明 (底栏, 运营资料硬要求)
    story.append(Paragraph(DISCLAIMER_EN, styles["Italic"]))
...
def generate_docx(
    ...
    # ── 显式水印 (节级水印文字) + 隐式水印 (core properties) ──
    _add_watermark(doc, WATERMARK_TEXT_CN)
    core = doc.core_properties
    core.author = GENERATOR_ID
    core.comments = (
        f"generator={GENERATOR_ID}; generated_at={datetime.now().isoformat(timespec='seconds')}"
...
def _add_feasibility_content(doc, data: Dict, metrics: Dict):
    ...
    _add_body(doc, f"{FORECAST_NOTE_CN}以上发电量基于假设条件推算，实际结果可能与预测存在偏差。")
    ...
    _add_styled_table(doc, ["年度", "净现金流"], cf_rows, col_widths=[6, 10],
                      caption="前10年净现金流预测")
    _add_body(doc, f"{FORECAST_NOTE_CN}现金流为模型推算值，受电价、运维成本等假设影响，非确定结果。")
...
def _add_investment_content(doc, data: Dict, metrics: Dict):
    ...
    _add_body(doc,
        ...
        f"预计 2025-2030 年复合增长率维持在 15-20%。{FORECAST_NOTE_CN}"
    )
...
def _add_appendix(doc, data: Dict, metrics: Dict):
    ...
    _add_heading(doc, "D. 数据溯源表", level=2)
    _add_body(doc, "下表列出报告关键数据项的来源标注；不确定或缺失的数据标「资料未及」，不作猜测。")
    _add_styled_table(
        doc, ["数据项", "来源标注"],
        _data_provenance_rows(data, lang="cn"),
        col_widths=[6, 10],
        caption="数据溯源表",
    )


def _add_end_disclaimer(doc):
    """文末固定免责声明段 (底栏, 全部分析输出强制携带)"""
    from docx.shared import Pt, RGBColor

    p = doc.add_paragraph()
    run = p.add_run(DISCLAIMER_CN)
    run.font.size = Pt(LAYOUT["font_size"]["caption"])
...
def generate_pdf(
    ...
    story.append(Paragraph(DISCLAIMER_CN, style_small))
    ...
    def on_page(canvas, doc):
        ...
        try:
            ...
            pass
        canvas.setFont(cn_font, 40)
        canvas.translate(A4[0] / 2, A4[1] / 2)
        canvas.rotate(45)
        canvas.drawCentredString(0, 0, WATERMARK_TEXT_CN)
        canvas.restoreState()
...
def _build_pdf_content(story, data, metrics, report_type, style_h1, style_h2, style_body, style_caption, cn_font, primary, accent, light_gray, table_counter):
    ...
    if report_type == "feasibility":
        ...
        story.append(make_table(["年度", "净现金流"], cf_rows, [6, 10],
                                caption="前5年净现金流预测"))
        story.append(Paragraph(
            f"{FORECAST_NOTE_CN}现金流为模型推算值，非确定结果。", style_body))
        ...
        story.append(Paragraph(
            f"2024年全国光伏新增装机超200GW。预计2025-2030年CAGR 15-20%。{FORECAST_NOTE_CN}", style_body))
...
def _build_pdf_appendix(story, data, metrics, style_h1, style_h2, style_body, cn_font, primary, light_gray, style_small, style_caption=None, table_counter=None):
    ...
    story.append(Paragraph("D. 数据溯源表", style_h2))
    story.append(Paragraph(
        "下表列出报告关键数据项的来源标注；不确定或缺失的数据标「资料未及」，不作猜测。",
        style_body))
    prov_rows = _data_provenance_rows(data, lang="cn")
    pt = Table([["数据项", "来源标注"]] + prov_rows, colWidths=[6 * cm, 10 * cm])
