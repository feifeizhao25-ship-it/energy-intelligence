...
export const TOOL_WHITELIST: Record<string, ToolDefinition> = {
    ...
    // ========== 选址辅助（只读） ==========
    // 这两个工具在 lib/ai/tool-executor.ts 里早已实现，
    // SiteWizard 也一直在调，但从未登记到白名单 ——
    // 新增的 /api/ai/tool 入口按白名单放行，不登记就会 403。
    explain_site_recommendation: {
        id: 'explain_site_recommendation',
        name: '选址方案解读',
        description: '把风光对比测算结果翻译成面向投资决策者的结构化解读',
        category: 'RESOURCE',
        riskLevel: 'READ_ONLY',
        requiresConfirmation: false,
        quotaType: 'AI_CALLS',
        ...
        parameters: {
            comparisonResult: {
                type: 'object',
                required: true,
                description: '来自 /api/calculate/comparison 的对比结果',
            },
        },
        enabled: true,
        version: '1.0',
    ...
    geocode_address: {
        ...
        requiredPermissions: [],
        parameters: {
            address: { type: 'string', required: true, description: '文字地址' },
        },
        enabled: true,
        version: '1.0',
    },
